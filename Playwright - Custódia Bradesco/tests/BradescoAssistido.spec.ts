...
test('Bradesco custódia assistido', async ({ page }) => {
  test.setTimeout(200000);
  let nomeNovo: string | undefined;
  let nomeNovoP: string | undefined;

  //Data
  const dataInformada = process.env.DATA!;
  const [dia, mes, ano] = dataInformada.split('/');
  const diaOntem = dia.padStart(2, '0');
  const mesOntem = mes.padStart(2, '0');
  const anoOntem = ano;
  const ontem = new Date(`${ano}-${mes}-${dia}`);
